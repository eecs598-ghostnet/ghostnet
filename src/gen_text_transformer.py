...
from train_transformer import Namespace
...
def greedy_search(model, txt_vocab, phoneme_vocab, corpus, device, k=1,
    ...
    for i in range(max_length + 1):
        # Generate example using greedy search
        word_tokens = [txt_vocab.stoi[word] for word in words]
        txt_input = torch.Tensor(word_tokens).view(1, -1).to(device).long()
    
        phonemes = [d.word2phonemes[word] for word in words]
    
        phoneme_tokens = [[phoneme_vocab.stoi[phoneme] for phoneme in word_phonemes] for word_phonemes in phonemes]
        phoneme_lengths = [len(word_phonemes) for word_phonemes in phonemes]
        phoneme_inputs = torch.ones(1, len(phoneme_lengths), max(phoneme_lengths)).to(device).long()
        for p in range(len(phoneme_lengths)):
            phoneme_inputs[0, p,:phoneme_lengths[p]] = torch.tensor(phoneme_tokens[p])
    
        phoneme_lengths = torch.tensor(phoneme_lengths).view(1, -1).to(device).long()
        opt = Namespace(src_pad = 1, device=device)
        txt_transformer_mask = create_context_mask(txt_input, opt)

        output = model.gen_word(txt_input, phoneme_inputs, txt_transformer_mask, phoneme_lengths)
        output_token = output.argmax().item()
        word = txt_vocab.itos[output_token]
        if word == '<pad>':
            continue # Ignore results that are just padding tokens.
...
def attn_beam_search(model, txt_vocab, phoneme_vocab, corpus, device, k=3,
    ...
    for word in words:
        ...
        #outputs = model(txt_input, phonemes_input, txt_lengths, phoneme_lengths)
        scores = model.gen_word(txt_input, phoneme_input, txt_transformer_mask, phoneme_lengths)
...
def gen_samples(seed_phrases, *args, **kwargs):
    for seed_words in seed_phrases:
        ...
        words = greedy_search(*args, **kwargs, seed_words=seed_words)
        print(words)
        print()
...
if __name__ == '__main__':
    ...
    model_weights_path = '../model/transformer/final3.pt'
    ...
    if seed_words is not None:
        greedy_search(model, txt_vocab, phoneme_vocab, corpus, device, seed_words=seed_words, max_length=50, k=1, show_options=True)
        #attn_beam_search(model, txt_vocab, phoneme_vocab, corpus, device, seed_words=seed_words, max_length=50, k=10, show_options=True)
        ...
        seed_phrases = [
            ...
            '<sos> ugh yea',
            '<sos> words embedded',
            '<sos> get money get new stuff',
            '<sos> another rhyme',
        ]
        seed_phrases = get_seed_phrases(100, dataloaders['test'], corpus)
        gen_samples(seed_phrases, model, txt_vocab, phoneme_vocab, corpus, device, max_length=100, k=10)
