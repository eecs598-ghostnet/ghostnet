# Code adapated from: https://www.kaggle.com/paultimothymooney/poetry-generator-rnn-markov/notebook
...
artist_file = '../bdata/nslyrics.txt'
...
def generate_lyrics(text_model, text_file):
	...
	print("Lyrics Length: " + str(lyriclength))
	count = 0
	...
	while len(bars) < lyriclength / 9 and count < lyriclength * 2 and len(bars) < 1000:
		...
		if type(bar) != type(None) and syllables(bar) < 1:
			...
			if bar not in bars and last_words.count(last_word) < 3:
				...
				count += 1
				if (len(bars) % 100 == 0):
					print("Finished with " + str(len(bars)) + " bars!")

	print("Finished generating bars!")
	return bars
